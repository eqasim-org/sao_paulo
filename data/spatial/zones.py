...
def execute(context):
    df_zones_hts = gpd.read_file("%s/Spatial/Zonas_2017_region.shp" % context.config("data_path"))
    df_zones_hts.crs = {"init" : "EPSG:29183"}    
    df_zones_hts = df_zones_hts[['NumeroZona', 'geometry']]
    df_zones_hts.columns = ["zone_id", "geometry"]
    
    df_zones_census = gpd.read_file("%s/Spatial/SC2010_RMSP_CEM_V3_merged.shp" % context.config("data_path"))
